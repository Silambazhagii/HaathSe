...
// Robust generation helper that falls back if a model experiences high demand or quota limits
async function generateContentWithFallback(payload) {
    ...
    for (const model of models) {
        try {
            ...
            if (errStr.includes('demand') || errStr.includes('quota') || errStr.includes('limit') || errStr.includes('unavailable') || errStr.includes('503') || errStr.includes('429')) {
                continue;
            } else {
                throw err;
            }
...
async function handleIncomingMedia(msg, mediaType) {
    ...
    try {
        ...
        let publicImageUrl = isVideo 
            ? "https://images.unsplash.com/photo-1582721478779-0ae163c05a60?auto=format&fit=crop&q=80&w=600" 
            : fileLink;
...
### Expected JSON Schema Output Structure:
{
  ...
  "authenticityStatus": "[Authenticity status]"
            ...
            contents: [
                {
                    role: 'user',
                    parts: [
                        { inlineData: { data: buffer.toString("base64"), mimeType: detectedMimeType } },
                        { text: prompt }
                    ]
                }
            ]
        ...
        // --- NATIVE IMAGE GENERATION (IMAGEN) FROM THE VIDEO SCAN ---
        let finalProductImageUrl = publicImageUrl;
        if (isVideo) {
            await bot.editMessageText(`🎨 Generating high-quality product photo from video description...`, { chat_id: chatId, message_id: statusMsg.message_id });
            try {
                const imagePrompt = `A high-end luxury e-commerce catalog studio photo of ${data.productName}, showcasing its ${data.craftHeritage} style. Isolated on a warm, minimalist stone/ivory colored neutral background, soft dramatic studio lighting, extremely detailed, professional photography.`;
                
                console.log("🎨 Triggering Imagen for prompt:", imagePrompt);
                ...
                if (imagenResponse.generatedImages && imagenResponse.generatedImages.length > 0) {
                    ...
                    if (supabase) {
                        const genFileName = `gen-${Date.now()}-${fileId}.jpg`;
                        const { data: uploadData, error: uploadError } = await supabase.storage
                            .from('product-images')
                            .upload(genFileName, imgBuffer, { contentType: 'image/jpeg' });
                        ...
                        if (!uploadError) {
                            ...
                            if (publicUrlData) {
                                finalProductImageUrl = publicUrlData.publicUrl;
                                console.log("✅ Successfully replaced video with generated high-quality photo:", finalProductImageUrl);
                            }
                        }
                    }
                }
            } catch (imageErr) {
                console.error("⚠️ Imagen generation failed, using beautiful category fallback photo:", imageErr.message || imageErr);
                // Fallback to high-quality curated category placeholders so the site doesn't show lava/blank spaces
                const style = String(data.craftHeritage || "").toLowerCase();
                if (style.includes("shawl") || style.includes("pashmina") || style.includes("weaving")) {
                    finalProductImageUrl = "https://images.unsplash.com/photo-1544816155-12df9643f363?auto=format&fit=crop&q=80&w=600"; // Beautiful cashmere scarf/textile studio photo
                } else if (style.includes("pottery") || style.includes("ceramic") || style.includes("vase")) {
                    finalProductImageUrl = "https://images.unsplash.com/photo-1612196808214-b8e1d6145a8c?auto=format&fit=crop&q=80&w=600"; // Blue pottery vase
                } else if (style.includes("saree") || style.includes("silk") || style.includes("textile")) {
                    finalProductImageUrl = "https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&q=80&w=600"; // Kanjivaram silk saree
                } else if (style.includes("wood") || style.includes("carving") || style.includes("ganesha")) {
                    finalProductImageUrl = "https://images.unsplash.com/photo-1582721478779-0ae163c05a60?auto=format&fit=crop&q=80&w=600"; // Wooden handicraft
                } else {
                    finalProductImageUrl = "https://images.unsplash.com/photo-1579783900882-c0d3dad7b119?auto=format&fit=crop&q=80&w=600"; // General Indian canvas painting
                }
            }
        }
        
        activeSessions[chatId].imageUrl = finalProductImageUrl;
...
*(Tip: Hold the microphone to confirm if this is Original or Replica, or to translate!)*`;
        ...
        bot.sendMessage(chatId, `❌ *System Alert: The AI could not process this file.*\n\n*Error details:* \`${errMsg}\`\n\nPlease try sending a shorter video or a photo.`);
    }
}
